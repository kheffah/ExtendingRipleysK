...
%
%   PURPOSE:
%   This script creates a randomly-located, variable-sized disk-shaped 
%   clusters (without overlap).
%   Signal-to-background ratio (SBR) varies as the aggregate-to-diffuse 
%   ratio (ADR) increases while the number of clusters is constant. 
%   the . Note that the signal is added to the background, so at SBR = 1.5,
%   the pixels belonging to aggregates will become background pixels plus 
%   1.5X background pixels (i.e. overall, 2.5X background pixels).
% 
%   SAMPLE INPUT:
...
NAgg = answer{8,1}; %Number of aggregates
...
NpAgg = NAgg .* sum(StrelSums(:)); %Number of pixels assigned to aggregates  
...
Parameters {ImNo+1,8} = 'Not pre-specified - calculated post-Hoc';

function [K,K_NC,Q01,Q99] = RipleysK2 (M,radius)
...
%
%   PURPOSE:
%   This function calculates Ripley's K-function for an irregular
%   grayscale field of view, thresholded against a background of zeros. 
%   It IGNORES zero pixels from the analysis. That is, it considers them to
%   be totally OUTSIDE the ROI, as opposed to being considered to be areas
%   "devoid" of particles.
%   It also calculated the critical
...
%
%   [K,K_NC,Q01,Q99] = RipleysK2 (im,radius);
%
%   SAMPLE OUTPUT:
% 
...
for i = radius+1:m+radius
    for j = radius+1:n+radius
        ...
        %
        %   IGNORING ZERO PIXELS FROM THE ORIGINAL IMAGE FROM EDGE
        %   CORRECTION
        %
...
N_Border = zeros(size(N_Binary));
 
for i = 2:(m+2*radius-1)
for j = 2:(n+2*radius-1)
